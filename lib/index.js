...
module.exports = ( function () {
    ...
    /* Core */
    Workbook.prototype.substituteForOsurvey = function ( sheetNumber, substitutionsData ) {
        var self = this;
        //Contain : name,id,
        ...
        var RowQuestion, CellQuestion;
        var RowType, CellType;
        var RowAnswerFormulation, CellAnswerFormulation;
        var RowAnswerNbRep, CellAnswerNbRep;
                ...
                if ( cell.attrib.t === "s" ) {
                    ...
                    self.extractPlaceholders( string ).forEach( function ( placeholder ) {
                        ...
                        if ( placeholder.type == "table" ) {
                            if ( placeholder.key == "formulation" ) {
                            ...
                            if ( placeholder.key == "answers.formulation" ) {

                                RowAnswerFormulation = row;
                                CellAnswerFormulation = cell;
                            }
                            if ( placeholder.key == "answers.nbRep" ) {

                                RowAnswerNbRep = row;
                                CellAnswerNbRep = cell;
                            }
                        }
                    } );
                };
        ...
        var firstQuestionInserted = false;
        var firstAnswerInserted = false;

        substitutionsData.questions.forEach( function ( question, idx ) {
            if ( !firstQuestionInserted ) {
                self.insertCellValue( CellQuestion, question.formulation );
                self.insertCellValue( CellType, question.type );

                firstQuestionInserted = true;
            }
            else {
                var cellsInsertedQ = 0;

                var newRow = self.cloneElement( RowQuestion, false );
                newRow.attrib.r = self.getCurrentRow( RowQuestion, rowsInserted + 2 );
                rowsInserted++;
                /*question.formulation*/
                var newCellFormulation = self.cloneElement( CellQuestion );
                newCellFormulation.attrib.r = self.joinRef( {
                    row: newRow.attrib.r,
                    col: self.splitRef( newCellFormulation.attrib.r ).col
                } );
                self.insertCellValue( newCellFormulation, question.formulation );
                /*question.type*/
                var newCellType = self.cloneElement( CellType );
                newCellType.attrib.r = self.joinRef( {
                    row: newRow.attrib.r,
                    col: self.splitRef( newCellType.attrib.r ).col
                } );
                self.insertCellValue( newCellType, question.type );

                newRow.append( newCellType );

                newRow.append( newCellFormulation );
            ...
            question.answers.forEach( function ( answer, idx ) {
                if ( !firstAnswerInserted ) {
                    self.insertCellValue( CellAnswerFormulation, answer.formulation );
                    self.insertCellValue( CellAnswerNbRep, answer.nbRep );
                    firstAnswerInserted = true;
                }
                else {

                    var cellsInsertedA = 0;

                    var newRow = self.cloneElement( RowAnswerFormulation, false );
                    newRow.attrib.r = self.getCurrentRow( RowAnswerFormulation, rowsInserted + 1 );
                    rowsInserted++;
                    /* answer.formulation */
                    var newCellFormulation = self.cloneElement( CellAnswerFormulation );
                    newCellFormulation.attrib.r = self.joinRef( {
                        row: newRow.attrib.r,
                        col: self.splitRef( newCellFormulation.attrib.r ).col
                    } );
                    self.insertCellValue( newCellFormulation, answer.formulation );
                    /* answer.nbRep */
                    var newCellNbRep = self.cloneElement( CellAnswerNbRep );
                    newCellNbRep.attrib.r = self.joinRef( {
                        row: newRow.attrib.r,
                        col: self.splitRef( newCellNbRep.attrib.r ).col
                    } );
                    self.insertCellValue( newCellNbRep, answer.nbRep );

                    newRow.append( newCellFormulation );
                    newRow.append( newCellNbRep );
